...
JIG_CFG_LOCAL_KEYS="housekeeping.cadence housekeeping.fetch housekeeping.trash_ttl housekeeping.abandoned_ttl housekeeping.stale_after git.worktree_root"
...
cfg() {
  ...
  [ -n "$value" ] || value=$(_cfg_read "$(jig_config_file)" "$key")
  printf '%s\n' "${value:-$default}"
}
