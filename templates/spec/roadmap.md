...
Destination: <one sentence: what is true when the last item is done>

## Phase 1 — <name>

Goal: <what this phase makes true>. Done when: <observable check>.

- [ ] `<task-id>` — <goal> (after: `<id>` — <one-line reason>)
- [ ] <planned item, not filed yet> — <goal>
- [ ] fog: <area> — <why it cannot be stated precisely yet>
...
1. <items that can run in parallel — separate worktrees>
2. <items that depend only on wave 1>
