...
| `base_branch` | `jig task start` | the branch the task was cut from and has to land on: `git.base_branch`, or the open epic of the spec the task links to (ADR-0038, ADR-0039). Checked with `git check-ref-format --branch` before it is written. Absent while the task is unstarted and in workspaces predating the field; every reader then uses `git.base_branch` (`jig_task_base`) |
