...
No level merges. Merging is your decision, always. The same levels apply to a specification's epic
...
- **The version.** You choose patch, minor or major when the epic is declared; at the finish the
  ...
  recorded.
- **Merging.** Merging the epic's final pull request is the release, and no level does it.

`jig spec ship` keeps the limits of `jig task ship` below: it never merges, force-pushes or skips
hooks, commits only what was staged, and refuses when task notes or runtime files are staged. A spec
