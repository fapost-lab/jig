...
<Accordion title="For contributors: how skill descriptions are tested">
...
| `jig-autopilot` | You hand a task over to be done without you ([Autopilot](/autopilot)) | Runs the task's whole route without waiting between stages, has a second agent review it, limits fixing to two attempts, and ends with the change handed over and a report. Given a phase of a roadmap instead of a task, it coordinates a wave of tasks built in parallel ([running a whole phase](/autopilot#running-a-whole-phase)). | Only at a stop: a design approval, a problem it could not fix, a decision nobody made, or a destructive step. |
| `jig-setup` | After `jig init`, or whenever you want to change what your agent may do on its own | Asks you, one question at a time and with a recommendation, how far the agent may take a finished task, whether it may merge and whether autopilot may run without asking; shows the whole `.ai/config.local.yaml` it would write ([personal settings](/configuration#let-the-agent-ask-you)). | Each setting, one at a time; your yes before anything is written — and an explicit yes for merging and for running without questions. |
