...
<Tabs>
  ...
  <Tab title="Windows">
    ...
    3. **Your first project**, if you agree — at most four questions: which folder, whether to make
       ...
       Some folders it refuses outright, and `-Yes` does not change that: your user folder
       (`C:\Users\<name>`), any folder above it, the root of a drive, and a folder that sits
       inside another Git repository — Jig would set itself up in that repository's top folder,
       not in the one you named. Make a folder for the project and use that; the refusal prints
       the two commands. Pasting the line from a fresh PowerShell window starts you in your user
       folder, so this is the usual case: type a folder name when it asks.
