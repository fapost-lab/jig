...
| `verify.busy_ttl` | `30m` | How long a running `jig verify` holds this clone. A second `jig verify` — in this checkout or in any worktree of it — waits for the first instead of competing with it for the machine, saying so while it waits. Past this age the record is ignored, so a run longer than this can be run over; raise it if your suite takes longer. `0` never waits. In CI the mechanism is off, because parallel jobs there are the point. |
...
|---|---|---|
| `generic` | always | Nothing: it checks no code. It fails when the folder is not a Git repository and otherwise reports a skip, so a project it is alone in hears that nothing verifies it. |
