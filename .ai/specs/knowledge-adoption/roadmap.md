...
- [x] `adopted-rules-stay-current` — Consolidation into sources: `jig-consolidate` edits the source, follows the project's own ADR
...
- [x] `adopted-rules-stay-current` — Copying untracked rule files: secret check that refuses until confirmed, the file shown
