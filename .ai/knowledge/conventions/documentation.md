...
paths:
  ...
  - AGENTS.md
  - templates/AGENTS.md
summary: How a behaviour claim earns its place in the docs — run it, paste the output, check it against the release branch — how a block of commands is ordered so the first line is the one to run, and how an instruction agents read is written so it beats the runtime's own default.
...
| A block of commands begins with the command the reader is meant to run, and a caveat that narrows or forbids one of the block's commands stands above the block, never below it. | `CONTRIBUTING.md` opened its Tests section with `bash tests/run.sh` — the whole suite, in parallel, one job per CPU — and left the caveat that a full run belongs to CI (`verify.full_run: ci`) thirteen lines lower. Eight agents each took the first runnable command on the page and ran it on one machine: a suite that takes five or six minutes alone took forty, two reviews stalled at 40 and 50 minutes, load average reached 364, and runs died with `Killed: 9` — for two hours that read as a defect in the code rather than a loaded machine. Nobody broke a rule; each reader did what the page taught, because reading order is stronger than content and a caveat under the block arrives after the command has already gone. Nothing checks this and, by `conventions/required-records.md`, nothing can: which command was meant to be called is the author's intent, held nowhere the machine can read, so the rule is neither computed nor refutable by the tree. It has the standing the `known-issues` entries have in this table — a reviewer notices it, or nobody does. |
...
| A release is cut from what is merged, never from an empty queue. Work still open is named in `docs/known-issues.mdx` and ordered on `docs/roadmap.mdx`; it does not hold the release. | The 0.16.0 task was filed with the condition "not before the queue is empty", and by the time it came to be run the queue held 22 active tasks with a standing decision not to cut it. The condition was therefore unreachable by construction, and a release rule that can never fire does not delay a release — it cancels it, while reading like prudence. What the condition was actually protecting against is real and has its own answer: a release must not describe as open what it already fixed, nor stay silent about what it did not. Both are answered by the two pages, which are written in the same pull request as the changelog, and neither needs the queue to be empty. |
...
| An error message the page quotes is reproduced from the oldest version a reader can be arriving from, not only from the one in front of you. | The paragraph about the first `jig upgrade` from an old Jig was written from a live case on 0.15.0, where the run ends `line 133: cmd: unbound variable` after its summary. Run from 0.14.0, the same upgrade ends `line 112: --reopen]: command not found`, exits 127 and prints no summary line at all — so a sentence promising only a varying line number would still have told that reader they had a different problem. One reproduction proves the message on one version; a page about upgrading spans a range of them, and both ends of that range have to be run. |
