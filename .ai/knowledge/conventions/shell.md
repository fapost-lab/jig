...
| Under `pipefail`, never pipe into a reader that can stop before the end of its input (`grep -q`, `head`, `awk '{ ...; exit }'`) when the writer is bash (`printf`, `echo`, a shell function) or may print more than a pipe buffer (`find`, `cut` of a file, `sort`). Test for a whole line with `jig_has_line <line> <text>` (common.sh; `jp_has_line` in profile.sh for profiles, a `case` in libraries sourced alone), a substring with `case "$text" in *"$s"*)`; let awk read to the end with a flag instead of `exit`; take a first line with `${var%%$'\n'*}` and a first `find` hit with `-print -quit`. `tests/dispatcher.t.sh` fails on any of `printf`, `echo`, `grep`, `find`, `sort`, `git` or `awk` piped into `grep -q`, `grep -m` or `head`, under `scripts/`, `profiles/`, `adapters/` or `.github/scripts/`. It joins line continuations before it looks, because a pipeline written across lines carries no single line with both halves, and it judges the stage that writes into the reader — which is why the `sed` form the rationale blesses passes. What proves the guard still sees anything is a planted sample, not a green run — `conventions/detectors.md`. | bash writes a pipe line by line, so `printf '%s\n' "$var"` is still writing when the reader quits; it dies of SIGPIPE, and `pipefail` turns 141 into a failed check — in a condition, a match silently becomes "no match". `jig spec plan` refused a valid roadmap about once in a hundred runs on Linux bash 5.2, never on macOS bash 3.2; `fm_list`, which stopped its awk at the end of the list, failed 47 of 12 000 calls; the shell profile's `printf "$names" \| grep -qF` over ~1 800 test names now and then called a filter empty and ran the full set. An external writer fills one pipe buffer per write, so `sed -n 's/^key: //p' file \| head -n 1` on a small file is safe. |
...
| Never let `grep`, `sed` or `awk` decide whether a line ends CRLF, and never assert a CR with them. Detect the ending by reading a line with the shell (`IFS= read -r`), which keeps the byte; count CRs with a byte tool (`tr -cd '\r' \| wc -c`); and re-apply endings once to a whole assembled stream instead of trusting each tool to pass a CR through. | Under Git Bash those three take the CR as part of the line separator and drop it before any pattern sees it: on a file whose every line ends CRLF, `grep -c '<CR>$'` answers 0 and `awk '{ print }'` prints the line back without it. `jig_section_write` read the ending from `grep … \| sed -n 1p`, so its `crlf` flag was always 0, and its awk head/tail passthrough dropped the rest — `jig upgrade` rewrote a Windows project's CRLF `AGENTS.md` as LF, the whole file, which is exactly what the section design promised not to do. The test meant to catch it asserted with `grep -c '<CR>$'`, which answers 0 on a *correct* file too: it could not pass on Windows whatever the code did, and so measured nothing anywhere (ADR-0037). |
...
| Any `rm -rf` or `mv` on a computed path is preceded by a validation that the path is inside `.ai/` and shaped as expected — or, for the deletions RULES.md names one by one, inside the bounded directory that exception allows. | RULES.md invariant; ADR-0006. The named exceptions are the installer, `jig spec new` and `jig upgrade`. The carry into a task worktree is a shape, not an exception: it *deletes* only inside `.ai/runtime/`, and the `mv` it uses to publish a path or take one back stays inside the worktree, checked with the same physical containment test before it runs. Each validates the root it is allowed to act under, and none may skip the check because another command ran one. |
...
| jig decides **which** repository it is in from where it was started: the dispatcher and `jig_require_repo` call `jig_clear_git_location_env` before any `git` read. | `git rev-parse --show-toplevel` asks the environment first and `git -C <dir>` does not override it, so `GIT_DIR` naming another repository sent every record jig wrote there and none here — measured (adr-20260924-a-checkout-records-what-is-happening-in-it, one layer below its own lesson). The cleared list is git(1) "The Git Repository", read rather than recalled, and goes together: clearing `GIT_DIR` while `GIT_OBJECT_DIRECTORY` stands commits here and writes the objects elsewhere. The line is location, not configuration — `GIT_CONFIG_GLOBAL` and its neighbours can redirect a work tree too and are left alone, because they are how a person configures git on purpose, and `tests/run.sh` uses one to isolate a run. A test runner owes the same list: a suite started from a shell that names another repository would otherwise run every test against it. |
